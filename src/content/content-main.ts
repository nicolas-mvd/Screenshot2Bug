...
(() => {
  ...
  function isTextLike(contentType = "") {
    return (
      !contentType ||
      /text|json|xml|javascript|typescript|html|css|svg|form-urlencoded/i.test(contentType)
    );
  }
    ...
    for (const [key, value] of formData.entries()) {
      values[key] =
        value instanceof File
          ? `[File: ${value.name}, ${value.type || "application/octet-stream"}, ${value.size} bytes]`
          : value;
    }
