...
function startPolling() {
  ...
  pollHandle = window.setInterval(async () => {
    ...
    if (!latest) return;
    const shouldRefreshReport = latest.status !== session.status || latest.recordingDataUrl !== session.recordingDataUrl || latest.screenshotDataUrl !== session.screenshotDataUrl || latest.consoleErrors.length !== session.consoleErrors.length;
    session = latest;
